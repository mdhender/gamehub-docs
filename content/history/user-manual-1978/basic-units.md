...
The possession of these units by a colony/ship[^1] determines the ability of the colony or ship to comply with an order.
...
a. Ration orders: Limit food consumption of a ship/colony.
b. Pay orders: Set pay rates, which remain constant until changed, for a ship/colony.
c. Draft orders: Recruit soldiers or trainees.
d. Assembly orders: Form construction or spy units. (Assembly orders have other functions as well.)
...
| --- | --- | --- |
| UNEMPLOYABLES | Child rearing and pregnant females, children, combat casualties, and the elderly. | All birth increases[^2] go in this category. They occur each quarter and are never less than 0.25% nor more than 2.5% of the total population. |
| UNSKILLED WORKERS | Those who do not require long apprenticeships or a great deal of training. | 2% of the unemployables become unskilled workers on any turn when the unemployable population is greater than 30% of the total population.[^3] All disabanded soldiers become unskilled workers. |
...
| TYPE | NO. OF PEOPLE IN UNIT | PAYMENT IN CONSUMER GOODS[^4] PER UNIT PER TURN | DEATH RATE (NON-COMBAT) PER TURN |
| --- | --- | --- | --- |
| Unemployables | 100 | 0.00 | 0.0625% |
| Unskilled workers | 100 | 0.125 | 0.0625% |
| Professionals | 100 | 0.375 | 0.0625% |
| Soldiers | 100 | 0.25 | 0.0625% |
| Spies | 200 + 1 professional unit + 1 soldier unit | 0.375 + 0.25 | 0.0625% |
| Construction | 200 + 1 professional unit + 1 unskilled worker unit | 0.375 + 0.125 | 0.0625% |
| Rebels | 100 | Payment and death rate are not applicable for rebels, as they are not an actual class. Their population is included in the other population segments. |  |
...
| ASSAULT CRAFT | are land/space vehicles used to invade colonies or ships. | 0.1 fuel unit per turn | 5 X TL[^5] |
...
| ENERGY WEAPONS | can be used in all combat situations except that of surface colony to surface colony. An energy weapon projects a powerful beam of concentrated energy. | 4 X TL per CR[^6] | 1O X TL |
...
| PRODUCTION UNIT | TECH LEVEL | ANNUAL PRODUCTION | TOTAL MASS OF UNIT | FUEL USE[^7] PER TURN |
| --- | --- | --- | --- | --- |
| MINES | TL-1 through TL-10 | 100 X TL[^8] = MU[^9] output | 10 + (2 X TL) | 0.5 units X TL |
...
Actions of factories are detailed under Vi Manufacturing, because of their complexity.
...
Consumer goods[^10] are produced by factories[^11] and used to pay the population[^12].
...
Each engine can propell 1,000 mass units times its TL through hyper space.
The total mass of a hyper engine is 45 X its TL.
...
The total mass of a space drive is 25 times its TL.
The space drive only uses fuel during combat; the number of fuel units used per combat round is equal to its \(\text{TL}^2\).
The speed of a space drive is important only during combat and is determined by its thrust factor.
Space drive \(\text{TL}^2 \times 1{,}000\) = thrust factor.
...
Sensors automatically inform a colony/ship of the number of ships in orbit and the approximate[^13] mass of each; the number of colonies in orbit, the approximate mass of each, and the approximate number of production units per colony.
...
The number of mass units that can be transferred each turn is equal to the transport's \(\text{TL}^2 \times 200\)[^14].
Transports require only fuel to be operational; the number of fuel units used per turn is equal to the transport's \(\text{TL}^2\) divided by ten, but is proportional to the percentage of the transport's capacity actually being used[^15].
...
The chart in the beginning of V Colonies And Ships shows the number of structural units needed for each type of colony, and for each ship.
...
[^1]: Colony/ship or ship/colony refers to either a ship or a colony.
[^2]: Increases are based on the amount of habitable land unpopulated, and inversely on the standard of living.
[^3]: "Total population" does not include rebels; see rebel description.
[^4]: Payrates for all population segments may be adjusted by the player.
[^5]: TL stands for technological level of the weapon unit.
[^6]: CR stands for combat round.
[^7]: farms or factories on orbiting colonies within the fifth orbit require no fuel; they use solar power.
[^8]: TL refers to the production unit's technological level.
[^9]: MU stands for mass unit.
[^10]: The total mass of a consumer goods unit is 0.6 mass units.
[^11]: See manufacturing.
[^12]: See Population Chart B.
[^13]: See Approximations.
[^14]: This does not apply during combat; see Combat.
[^15]: Fuel used in combat is listed under Combat.
